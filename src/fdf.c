...
/*   Updated: 2025/04/03 14:29:36 by asemykin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	close_window(t_data *param)
{
	...
	free(param->fdf.mlx);
	free_iar(param->fdf.iar, param->fdf.col);
	exit(0);
	return (0);
}
...
/*
mlx_init();
	initializes the MiniLibX connection between the program
	and the graphical system.

	it returns a pointer to the mlx instance, which is necessary for
	creating windows, rendering images and handling events.
*/